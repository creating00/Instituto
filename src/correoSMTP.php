...
$rs = mysqli_query($conexion, "SELECT nombre, apellido, email FROM alumno WHERE dni='$dni'");
  ...
  if (empty($sede) || empty($dni) || empty($fecha)) {
    echo '<script language="javascript">';
    echo 'alert("Nose pudo enviar el Correo");';
    echo '</script>';
}else{  
...
"Informacion:
    ...
    <a href='http://sistemaeduser.com/src/pdf/reporte.php?dni=$dni&curso=$Mensaje&sede=$sede'class='btn btn-success'><i class='fas fa-edit'></i>Ver</a>

      <br><p>No necesitas clave<p>
      <p>Curso: $Mensaje<p>
      <p>Fecha de Inicio: $fecha <p>
      <p hidden>Sede: $sede <p>
      Información para el Alumno: La suma de la inscripcón solamente será devuelta si el curso en cuestión no comienza.
