...
if(isset($_POST['cobrar_cuota']))
	{
        ...
        //validar que no esten los campos vacios de cuota
        $alert = "";
        if (empty($total) || empty($idcuotas1) ) {
          echo '<script language="javascript">';
          echo 'alert("Campo total Vacio");';
          echo '</script>';
        }else {

        $rs = mysqli_query($conexion, "UPDATE cuotas SET fecha='$fechacuota', interes='$interes', total='$total', condicion='PAGADO', mediodepago='Efectivo', mediodepago='$mediodePago', idusuario='$idusuario1' WHERE idcuotas='$idcuotas1'");
        if ($rs) {
            echo '<script language="javascript">';
...
if(isset($_POST['mi_busqueda_inscripcion']))
	...
	';
        ...
        $idin=0;
        if($sede=="GENERAL"){
        $resultados = mysqli_query($conexion, "SELECT idinscripcion, alumno.dni, alumno.nombre'alumno', curso.nombre'curso', profesor.nombre'profesor',curso.precio, fecha, fechacomienzo, inscripcion.estado, curso.duracion, curso.tipo FROM inscripcion
            ...
            WHERE alumno.dni='$dni' AND usuario.idusuario='$idusuario' ORDER BY idinscripcion DESC");
        }else{
...
if(isset($_POST['mi_busqueda_cuotas']))
	...
	';
        ...
        //traer cuotas
        $resultados = mysqli_query($conexion, "SELECT idcuotas, fecha, cuota, mes, año, importe, interes, total, condicion, usuario.usuario, idexamen FROM cuotas
        INNER JOIN usuario on cuotas.idusuario=usuario.idusuario WHERE idinscripcion='$idinscripcion'");
        ...
        while($consulta = mysqli_fetch_array($resultados))
        {
            ...
            echo "<td><a href='pdf/imprimirCuota.php?id=".$consulta['idcuotas']." 'target='_blank''><i class='fa fa-print' aria-hidden='true'></i></a></td>";
            echo "<td><a href='eliminar_cuota.php?id=".$consulta['idcuotas']."'><i class='fas fa-trash-alt'></i> </a></td>";
...
if(isset($_POST['mostrar_cobros']))
	...
	';
        ...
        echo "<h5>Total recaudado Cuotas mas Examen: $ $retotal </h5>";
        ...
        echo "<h6><center>Lista de Alumnos Cobros Cuotas mas Examen</center></h6>";
}
...
$('#tablaCuotas tr').on('click', function(){
  ...
  $('#mes').val(dato6);
  var dato3 = $(this).find('td:nth-child(6)').html();
  $('#importe').val(dato3);
  var dato4 = $(this).find('td:nth-child(7)').html();
  $('#interes').val(dato4);
